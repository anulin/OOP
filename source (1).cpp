...
class Puzzle {
 ...
 public :
    ...
    Puzzle& operator = (const Puzzle& secPuz) {
        puzzle = secPuz.puzzle;
        curDist = secPuz.curDist;
        heurDist = secPuz.heurDist;
        lastMove = secPuz.lastMove;
        posOfZero = secPuz.posOfZero;
    }
...
class PuzzleSolver {
 private:
    ...
    std::priority_queue<Puzzle<PSIZE>, std::deque<Puzzle<PSIZE>>, CompBySumDist> priorQueue;
    std::set<Puzzle<PSIZE>, CompByOrder> coveredVert;
    std::vector<char> reqMovements;
    short minDist;
    short tmpForSwap;
    ...
    void moveZeroUp() {
        if (curPuzzle.getLastMove() != 'U') {
            if (posOfZero / PSIZE > 0) {
                tmpPuzz = curPuzzle;
                tmpForSwap = tmpPuzz[posOfZero - PSIZE];
                tmpPuzz[posOfZero - PSIZE] = 0;
                tmpPuzz[posOfZero] = tmpForSwap;
                tmpPuzz.incrCurDist();
                if (coveredVert.find(tmpPuzz) == coveredVert.end()) {
                    priorQueue.emplace(tmpPuzz, 'D', tmpPuzz.getCurDist(),
                                       heurDist(tmpPuzz), posOfZero - PSIZE);
                }
            }
        }
    }
    ...
    void moveZeroDown() {
        if (curPuzzle.getLastMove() != 'D') {
            if (posOfZero / PSIZE < PSIZE - 1) {
                ...
                tmpPuzz.incrCurDist();
                if (coveredVert.find(tmpPuzz) == coveredVert.end()) {
                    priorQueue.emplace(tmpPuzz, 'U', tmpPuzz.getCurDist(),
                                       heurDist(tmpPuzz), posOfZero + PSIZE);
                }
            }
        }
    ...
    void moveZeroRight() {
        if (curPuzzle.getLastMove() != 'R') {
            if (posOfZero % PSIZE < PSIZE - 1) {
                tmpPuzz = curPuzzle;
                tmpForSwap = tmpPuzz[posOfZero + 1];
                tmpPuzz[posOfZero + 1] = 0;
                tmpPuzz[posOfZero] = tmpForSwap;
                tmpPuzz.incrCurDist();
                if (coveredVert.find(tmpPuzz) == coveredVert.end()) {
                    priorQueue.emplace(tmpPuzz, 'L', tmpPuzz.getCurDist(),
                                       heurDist(tmpPuzz), posOfZero + 1);
                }
            }
        }
    }
    ...
    void moveZeroLeft() {
        if (curPuzzle.getLastMove() != 'L') {
            if (posOfZero % PSIZE > 0) {
                ...
                tmpPuzz.incrCurDist();
                if (coveredVert.find(tmpPuzz) == coveredVert.end()) {
                    priorQueue.emplace(tmpPuzz, 'R', tmpPuzz.getCurDist(),
                                       heurDist(tmpPuzz), posOfZero - 1);
                }
            }
        }
...
short manhattanDistance(const Puzzle<PSIZE>& puzzle) {
    ...
    for (short i = 1; i < PSIZE * PSIZE + 1; ++i) {
        ...
        xDif = (i - 1) % PSIZE > (puzzle[i - 1] - 1) % PSIZE ?
        (i - 1) % PSIZE - (puzzle[i - 1] - 1) % PSIZE :
            (puzzle[i - 1] - 1) % PSIZE - (i - 1) % PSIZE;
        yDif = (i - 1) / PSIZE > (puzzle[i - 1] - 1) / PSIZE ?
        (i - 1) / PSIZE - (puzzle[i - 1] - 1) / PSIZE :
            (puzzle[i - 1] - 1) / PSIZE - (i - 1) / PSIZE;
        distance += xDif;
        distance += yDif;
